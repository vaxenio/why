...
func (ins *inspector) resolveDeps(f *parsedFile, ancestors []*parsedFile, depth int) {
	...
	for _, soname := range f.needed {
		resolved := ins.resolve(soname, f, ancestors)
		if resolved == "" {
...
func (ins *inspector) resolve(soname string, f *parsedFile, ancestors []*parsedFile) string {
