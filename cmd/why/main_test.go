...
func TestRun(t *testing.T) {
	cases := []struct {
		...
		{name: "run missing target file", args: []string{"run", "--json", "target"}, stream: "stderr", wantSub: "why: run: target not found", wantCode: 1},
		{name: "run without target fails", args: []string{"run"}, stream: "stderr", wantSub: "why: run: missing target", wantCode: 1},
		{name: "run target after -- separator is missing", args: []string{"run", "--", "-x"}, stream: "stderr", wantSub: "why: run: target not found", wantCode: 1},
		{name: "inspect missing target file", args: []string{"inspect", "--json", "target"}, stream: "stderr", wantSub: "why: inspect: target not found", wantCode: 1},
		...
		{name: "report missing input", args: []string{"report", "--json", "target"}, stream: "stderr", wantSub: "why: report: input not found", wantCode: 1},
